  ...
  fetch('/api/settings').then(function (r) { return r.json(); }).then(function (cfg) {
    ...
    root.appendChild(section('Автоматическая перезагрузка', h('div', { class: 'set-block' }, [
      ...
      h('p', { class: 'set-hint', text:
        ...
        'в сутки. Свою машину дашборд перезагружает последней и только когда в окне ' +
        'больше некого — пока она выключена, некому ни перезагружать остальных, ни ' +
        'заметить, что она не вернулась. Перед каждой перезагрузкой уходит сообщение ' +
        'в Telegram; про свою — до команды и только если оно доставлено.' }),
      h('div', { class: 'set-row' }, [
